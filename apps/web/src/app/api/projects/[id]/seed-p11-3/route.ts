/**
 * POST /api/projects/[id]/seed-p11-3
 *
 * Seeds the Build OS P11.3 Historical Reality Reconciliation Audit.
 *
 * MODE: STRICT AUDIT — NO FEATURE BUILD — REALITY ONLY
 *
 * Creates: 1 epic, 7 features, 28 tasks across workstreams WS1–WS7.
 * Audits 387 tasks across P6A–P11.2 for core project feb25dda.
 * First task in WS1 starts as 'ready'. All others start as 'pending'.
 * Idempotent: returns 409 if P11.3 epic already exists.
 *
...
import { BUILD_OS_ROADMAP_P11_3, P11_3_EPIC_TITLE, ROADMAP_P11_3_SUMMARY } from '@/data/build-os-roadmap-p11-3'
...
export async function POST(
  ...
  try {
    ...
    const { data: existingEpic } = await admin
      ...
      .eq('title', P11_3_EPIC_TITLE)
      .limit(1)

    if (existingEpic && existingEpic.length > 0) {
      return NextResponse.json(
        { error: 'P11.3 roadmap already seeded', code: 'ALREADY_SEEDED' },
        { status: 409 }
      )
    }
    ...
    for (const epicDef of BUILD_OS_ROADMAP_P11_3) {
      ...
      if (epicErr || !epicRow) {
        console.error('[seed-p11-3] Failed to insert epic:', epicErr)
        return NextResponse.json(
      ...
      for (const featDef of epicDef.features) {
        ...
        if (featErr || !featRow) {
          console.error('[seed-p11-3] Failed to insert feature:', featErr)
          return NextResponse.json(
        ...
        const taskRows = featDef.tasks.map((taskDef, idx) => {
          ...
          // Extract workstream context from task context_payload
          const taskContext = (taskDef as any).context_payload || {}
          ...
          return {
            ...
            slug: taskSlug,
            agent_role: (taskDef as any).role || 'analyst',
            task_type: (taskDef as any).task_type || 'audit',
            priority: (taskDef as any).priority || 'medium',
            order_index: taskDef.order_index,
            estimated_cost_usd: (taskDef as any).estimated_cost_usd ?? 0.05,
            ...
            context_payload: {
              source: 'p11_3_audit',
              phase: '11.3',
              workstream: taskContext.workstream || `WS${featDef.order_index + 1}`,
              batch: taskContext.batch || 'unknown',
        ...
        if (tasksErr) {
          console.error('[seed-p11-3] Failed to insert tasks:', tasksErr)
          return NextResponse.json(
    ...
    console.log(`[seed-p11-3] Seeded ${totalTasksSeeded} tasks for project ${projectId}`)
    ...
    return NextResponse.json({
      ...
      epics: seededEpics,
      summary: {
        ...ROADMAP_P11_3_SUMMARY,
        tasks_seeded: totalTasksSeeded,
        features_seeded: totalFeaturesSeeded,
        base_order_index: baseOrderIndex,
        audit_scope: '387 tasks audited across P6A–P11.2',
      },
      first_task_status: 'ready',
      message: `P11.3 Historical Reality Reconciliation Audit seeded successfully.`,
    })

  } catch (err) {
    console.error('[seed-p11-3] Unexpected error:', err)
    return NextResponse.json(
