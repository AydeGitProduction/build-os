...
export function IrisPreviewPanel({ data, readiness, isBuilding, onAssumptionAction, onConfirm, isConfirming }: IrisPreviewPanelProps) {
    ...
    <div className="iris-pp">
      ...
      {state === 'PREVIEW' && data && (
        <div className="iris-pp-content" key={highlightKey}>
          ...
          {/* Confirm & Start Build */}
          {readiness >= 90 && onConfirm && (
            <div className="iris-pp-section iris-pp-confirm-section">
              <button
                onClick={onConfirm}
                disabled={isConfirming}
                className="iris-confirm-btn"
              >
                {isConfirming ? 'Starting Build…' : '🚀 Confirm & Start Build'}
              </button>
            </div>
          )}
        </div>
