...
function blueprintToPreview(
  ...
  const coreCount = (bp.blueprint_features || []).filter(
    (f) => f.is_core || f.priority === 'critical'
  ).length
      ...
      try {
        ...
        if (!pid) {
          ...
          const res = await fetch('/api/projects', {
            ...
            body: JSON.stringify({ name: 'New Project', workspace_id: workspaceId, project_type: 'saas' }),
          })
      ...
      <div className="iris-split">
        ...
        <div className="iris-preview-pane">
          <IrisPreviewPanel
            data={previewData}
            readiness={readiness}
            isBuilding={isTyping || (irisComplete && !blueprint)}
