...
// KEY INSIGHT: n8n commits when code block first line = // apps/web/src/path
// The path comment is the trigger — not the heading level
const tasks = [
  {
...
Replace the stub with this implementation. The FIRST LINE of your code block must be the file path as a comment (this triggers automated deployment):

\`\`\`typescript
// apps/web/src/app/api/projects/[id]/draft-preview/route.ts
...
Output this complete file. The first line of the code block MUST be the // path comment.`
  },
  {
...
\`\`\`tsx
// apps/web/src/app/(app)/projects/[id]/wizard/page.tsx
...
DO NOT touch components/Sidebar.tsx (orphaned). Only modify layout/Sidebar.tsx.

Add a WorkspaceSwitcher inline component before the main export, and render it in the sidebar JSX below the logo and above the nav links. The FIRST LINE of your code block must be the file path as a comment (this triggers automated deployment).

Read the existing Sidebar.tsx file to understand its current structure, then output the COMPLETE modified file with WorkspaceSwitcher added. The output must start with:

\`\`\`tsx
// apps/web/src/components/layout/Sidebar.tsx
[complete file content with WorkspaceSwitcher added]
\`\`\`

WorkspaceSwitcher requirements:
- Fetch workspaces from /api/workspaces on mount
- Show current workspace name with a chevron icon
- On click, open dropdown showing all workspaces
- Each workspace item navigates to /dashboard on click
- Use Tailwind classes consistent with existing zinc/slate sidebar palette
- Do not remove or modify any existing nav items, project section, or footer

The first line of the code block must be // apps/web/src/components/layout/Sidebar.tsx`
  }
]
...
async function run() {
  for (const task of tasks) {
    ...
    console.log('  ✓ Reset (// path comment pattern)')
  ...
  console.log('\nAll tasks re-dispatched with // path comment trigger.')
}
